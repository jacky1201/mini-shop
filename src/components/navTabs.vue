<template>
  <scroll-view scroll-x class="nav-scroll" :show-scrollbar="false" :scroll-into-view="`nav${modelValue}`" scroll-with-animation>
    <view class="nav-list">
      <view
        v-for="item in tabs"
        :key="item.key"
...
<script lang="ts" setup>
  ...
  // 当前选中的导航项
  const activeNavId = ref(modelValue.value)
  // 监听外部传入的默认选中项变化
  watch(activeNavId, (newVal) => {
